...
<?php include_once "r04_pengeluaranoldsmryinfo.php" ?>
<?php
...
$r04_pengeluaranold_summary = NULL; // Initialize page object first

class crr04_pengeluaranold_summary extends crr04_pengeluaranold {
	...
	// Page object name
	var $PageObjName = 'r04_pengeluaranold_summary';
	...
	function __construct() {
		...
		// Table object (r04_pengeluaranold)
		if (!isset($GLOBALS["r04_pengeluaranold"])) {
			$GLOBALS["r04_pengeluaranold"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["r04_pengeluaranold"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'r04_pengeluaranold', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fr04_pengeluaranoldsummary";
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'r04_pengeluaranold');
...
//		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewrExportLink ewEmail\" title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_r04_pengeluaranold\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_r04_pengeluaranold',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fr04_pengeluaranoldsummary\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fr04_pengeluaranoldsummary\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fr04_pengeluaranoldsummary\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function GetExtendedFilter() {
		...
		// Reset search command
			...
			if ($this->GetDropDownValue($this->maingroup_nama)) {
				$bSetupFilter = TRUE;
			} elseif ($this->maingroup_nama->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_r04_pengeluaranold_maingroup_nama'])) {
				$bSetupFilter = TRUE;
			}
			...
			if ($this->GetDropDownValue($this->subgroup_nama)) {
				$bSetupFilter = TRUE;
			} elseif ($this->subgroup_nama->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_r04_pengeluaranold_subgroup_nama'])) {
				$bSetupFilter = TRUE;
			}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_r04_pengeluaranold_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_r04_pengeluaranold_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_r04_pengeluaranold_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_r04_pengeluaranold_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_r04_pengeluaranold_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_r04_pengeluaranold_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_r04_pengeluaranold_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_r04_pengeluaranold_' . $parm] = $sv;
		$_SESSION['so_r04_pengeluaranold_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_r04_pengeluaranold_' . $parm] = $sv1;
		$_SESSION['so_r04_pengeluaranold_' . $parm] = $so1;
		$_SESSION['sc_r04_pengeluaranold_' . $parm] = $sc;
		$_SESSION['sv2_r04_pengeluaranold_' . $parm] = $sv2;
		$_SESSION['so2_r04_pengeluaranold_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_r04_pengeluaranold_$parm"] = "";
		$_SESSION["rf_r04_pengeluaranold_$parm"] = "";
		$_SESSION["rt_r04_pengeluaranold_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->FieldByParm($parm);
		$fld->SelectionList = @$_SESSION["sel_r04_pengeluaranold_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_r04_pengeluaranold_$parm"];
		$fld->RangeTo = @$_SESSION["rt_r04_pengeluaranold_$parm"];
	}
...
// Create page object
if (!isset($r04_pengeluaranold_summary)) $r04_pengeluaranold_summary = new crr04_pengeluaranold_summary();
if (isset($Page)) $OldPage = $Page;
$Page = &$r04_pengeluaranold_summary;
...
// Create page object
var r04_pengeluaranold_summary = new ewr_Page("r04_pengeluaranold_summary");

// Page properties
r04_pengeluaranold_summary.PageID = "summary"; // Page ID
var EWR_PAGE_ID = r04_pengeluaranold_summary.PageID;
</script>
<?php if (!$Page->DrillDown && !$grDashboardReport) { ?>
<script type="text/javascript">

// Form object
var CurrentForm = fr04_pengeluaranoldsummary = new ewr_Form("fr04_pengeluaranoldsummary");

// Validate method
fr04_pengeluaranoldsummary.Validate = function() {
...
fr04_pengeluaranoldsummary.Form_CustomValidate = 
...
fr04_pengeluaranoldsummary.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fr04_pengeluaranoldsummary.ValidateRequired = false; // No JavaScript validation
<?php } ?>

// Use Ajax
fr04_pengeluaranoldsummary.Lists["sv_maingroup_nama"] = {"LinkField":"sv_maingroup_nama","Ajax":true,"DisplayFields":["sv_maingroup_nama","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
fr04_pengeluaranoldsummary.Lists["sv_subgroup_nama"] = {"LinkField":"sv_subgroup_nama","Ajax":true,"DisplayFields":["sv_subgroup_nama","","",""],"ParentFields":["sv_maingroup_nama"],"FilterFields":["sv_maingroup_nama"],"Options":[],"Template":""};
</script>
<?php } ?>
<?php if (!$Page->DrillDown && !$grDashboardReport) { ?>
<script type="text/javascript">
...
<?php if (!$Page->DrillDown && !$grDashboardReport) { ?>
<!-- Search form (begin) -->
<form name="fr04_pengeluaranoldsummary" id="fr04_pengeluaranoldsummary" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fr04_pengeluaranoldsummary_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<script type="text/javascript">
fr04_pengeluaranoldsummary.Lists["sv_maingroup_nama"].Options = <?php echo ewr_ArrayToJson($Page->maingroup_nama->LookupFilterOptions) ?>;
</script>
</span>
</div>
</div>
...
<script type="text/javascript">
fr04_pengeluaranoldsummary.Lists["sv_subgroup_nama"].Options = <?php echo ewr_ArrayToJson($Page->subgroup_nama->LookupFilterOptions) ?>;
</script>
</span>
</div>
</div>
...
<script type="text/javascript">
fr04_pengeluaranoldsummary.Init();
fr04_pengeluaranoldsummary.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php include "r04_pengeluaranoldsmrypager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
</div>
<span data-class="tpb<?php echo $Page->GrpCount-1 ?>_r04_pengeluaranold"><?php echo $Page->PageBreakContent ?></span>
<?php } ?>
...
<?php } ?>
<!-- Report grid (begin) -->
<div id="gmp_r04_pengeluaranold" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="maingroup_nama"><div class="r04_pengeluaranold_maingroup_nama"><span class="ewTableHeaderCaption"><?php echo $Page->maingroup_nama->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="maingroup_nama">
<?php if ($Page->SortUrl($Page->maingroup_nama) == "") { ?>
		<div class="ewTableHeaderBtn r04_pengeluaranold_maingroup_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->maingroup_nama->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r04_pengeluaranold_maingroup_nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->maingroup_nama) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->maingroup_nama->FldCaption() ?></span>
...
<?php if ($Page->subgroup_nama->Visible) { ?>
	...
	<?php } else { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="subgroup_nama"><div class="r04_pengeluaranold_subgroup_nama"><span class="ewTableHeaderCaption"><?php echo $Page->subgroup_nama->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="subgroup_nama">
<?php if ($Page->SortUrl($Page->subgroup_nama) == "") { ?>
		<div class="ewTableHeaderBtn r04_pengeluaranold_subgroup_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->subgroup_nama->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r04_pengeluaranold_subgroup_nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->subgroup_nama) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->subgroup_nama->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="tanggal"><div class="r04_pengeluaranold_tanggal"><span class="ewTableHeaderCaption"><?php echo $Page->tanggal->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="tanggal">
<?php if ($Page->SortUrl($Page->tanggal) == "") { ?>
		<div class="ewTableHeaderBtn r04_pengeluaranold_tanggal">
			<span class="ewTableHeaderCaption"><?php echo $Page->tanggal->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r04_pengeluaranold_tanggal" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->tanggal) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->tanggal->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="supplier_nama"><div class="r04_pengeluaranold_supplier_nama"><span class="ewTableHeaderCaption"><?php echo $Page->supplier_nama->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="supplier_nama">
<?php if ($Page->SortUrl($Page->supplier_nama) == "") { ?>
		<div class="ewTableHeaderBtn r04_pengeluaranold_supplier_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->supplier_nama->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r04_pengeluaranold_supplier_nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->supplier_nama) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->supplier_nama->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="nonota"><div class="r04_pengeluaranold_nonota"><span class="ewTableHeaderCaption"><?php echo $Page->nonota->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="nonota">
<?php if ($Page->SortUrl($Page->nonota) == "") { ?>
		<div class="ewTableHeaderBtn r04_pengeluaranold_nonota">
			<span class="ewTableHeaderCaption"><?php echo $Page->nonota->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r04_pengeluaranold_nonota" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->nonota) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->nonota->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="barang_nama"><div class="r04_pengeluaranold_barang_nama"><span class="ewTableHeaderCaption"><?php echo $Page->barang_nama->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="barang_nama">
<?php if ($Page->SortUrl($Page->barang_nama) == "") { ?>
		<div class="ewTableHeaderBtn r04_pengeluaranold_barang_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->barang_nama->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r04_pengeluaranold_barang_nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->barang_nama) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->barang_nama->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="banyaknya"><div class="r04_pengeluaranold_banyaknya" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->banyaknya->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="banyaknya">
<?php if ($Page->SortUrl($Page->banyaknya) == "") { ?>
		<div class="ewTableHeaderBtn r04_pengeluaranold_banyaknya" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->banyaknya->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r04_pengeluaranold_banyaknya" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->banyaknya) ?>',1);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->banyaknya->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="barang_satuan"><div class="r04_pengeluaranold_barang_satuan"><span class="ewTableHeaderCaption"><?php echo $Page->barang_satuan->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="barang_satuan">
<?php if ($Page->SortUrl($Page->barang_satuan) == "") { ?>
		<div class="ewTableHeaderBtn r04_pengeluaranold_barang_satuan">
			<span class="ewTableHeaderCaption"><?php echo $Page->barang_satuan->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r04_pengeluaranold_barang_satuan" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->barang_satuan) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->barang_satuan->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="harga"><div class="r04_pengeluaranold_harga" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->harga->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="harga">
<?php if ($Page->SortUrl($Page->harga) == "") { ?>
		<div class="ewTableHeaderBtn r04_pengeluaranold_harga" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->harga->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r04_pengeluaranold_harga" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->harga) ?>',1);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->harga->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Jumlah"><div class="r04_pengeluaranold_Jumlah" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->Jumlah->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Jumlah">
<?php if ($Page->SortUrl($Page->Jumlah) == "") { ?>
		<div class="ewTableHeaderBtn r04_pengeluaranold_Jumlah" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->Jumlah->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r04_pengeluaranold_Jumlah" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Jumlah) ?>',1);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->Jumlah->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
		<span class="ewSummaryCaption r04_pengeluaranold_maingroup_nama"><span class="ewTableHeaderCaption"><?php echo $Page->maingroup_nama->FldCaption() ?></span></span>
<?php } else { ?>
	<?php if ($Page->SortUrl($Page->maingroup_nama) == "") { ?>
		<span class="ewSummaryCaption r04_pengeluaranold_maingroup_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->maingroup_nama->FldCaption() ?></span>
		</span>
	<?php } else { ?>
		<span class="ewTableHeaderBtn ewPointer ewSummaryCaption r04_pengeluaranold_maingroup_nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->maingroup_nama) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->maingroup_nama->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->maingroup_nama->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->maingroup_nama->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</span>
	<?php } ?>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_r04_pengeluaranold_maingroup_nama"<?php echo $Page->maingroup_nama->ViewAttributes() ?>><?php echo $Page->maingroup_nama->GroupViewValue ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
		<span class="ewSummaryCaption r04_pengeluaranold_subgroup_nama"><span class="ewTableHeaderCaption"><?php echo $Page->subgroup_nama->FldCaption() ?></span></span>
<?php } else { ?>
	<?php if ($Page->SortUrl($Page->subgroup_nama) == "") { ?>
		<span class="ewSummaryCaption r04_pengeluaranold_subgroup_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->subgroup_nama->FldCaption() ?></span>
		</span>
	<?php } else { ?>
		<span class="ewTableHeaderBtn ewPointer ewSummaryCaption r04_pengeluaranold_subgroup_nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->subgroup_nama) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->subgroup_nama->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->subgroup_nama->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->subgroup_nama->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</span>
	<?php } ?>
<?php } ?>
		<?php echo $ReportLanguage->Phrase("SummaryColon") ?>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_r04_pengeluaranold_subgroup_nama"<?php echo $Page->subgroup_nama->ViewAttributes() ?>><?php echo $Page->subgroup_nama->GroupViewValue ?></span>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_r04_pengeluaranold_maingroup_nama"<?php echo $Page->maingroup_nama->ViewAttributes() ?>><?php echo $Page->maingroup_nama->GroupViewValue ?></span></td>
	<?php } ?>
<?php } ?>
<?php if ($Page->subgroup_nama->Visible) { ?>
	<?php if ($Page->subgroup_nama->ShowGroupHeaderAsRow) { ?>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_r04_pengeluaranold_subgroup_nama"<?php echo $Page->subgroup_nama->ViewAttributes() ?>><?php echo $Page->subgroup_nama->GroupViewValue ?></span></td>
	<?php } ?>
<?php } ?>
<?php if ($Page->tanggal->Visible) { ?>
