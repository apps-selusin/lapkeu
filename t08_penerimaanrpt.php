...
<?php include_once "t08_penerimaanrptinfo.php" ?>
<?php
...
$t08_penerimaan_rpt = NULL; // Initialize page object first

class crt08_penerimaan_rpt extends crt08_penerimaan {
	...
	// Page object name
	var $PageObjName = 't08_penerimaan_rpt';
	...
	function __construct() {
		...
		if (!isset($GLOBALS["t08_penerimaan"])) {
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 't08_penerimaan', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption ft08_penerimaanrpt";
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 't08_penerimaan');
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewrExportLink ewEmail\" title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_t08_penerimaan\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_t08_penerimaan',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"ft08_penerimaanrpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"ft08_penerimaanrpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"ft08_penerimaanrpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_t08_penerimaan_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_t08_penerimaan_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_t08_penerimaan_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_t08_penerimaan_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_t08_penerimaan_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_t08_penerimaan_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_t08_penerimaan_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_t08_penerimaan_' . $parm] = $sv;
		$_SESSION['so_t08_penerimaan_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_t08_penerimaan_' . $parm] = $sv1;
		$_SESSION['so_t08_penerimaan_' . $parm] = $so1;
		$_SESSION['sc_t08_penerimaan_' . $parm] = $sc;
		$_SESSION['sv2_t08_penerimaan_' . $parm] = $sv2;
		$_SESSION['so2_t08_penerimaan_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_t08_penerimaan_$parm"] = "";
		$_SESSION["rf_t08_penerimaan_$parm"] = "";
		$_SESSION["rt_t08_penerimaan_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->FieldByParm($parm);
		$fld->SelectionList = @$_SESSION["sel_t08_penerimaan_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_t08_penerimaan_$parm"];
		$fld->RangeTo = @$_SESSION["rt_t08_penerimaan_$parm"];
	}
...
// Create page object
if (!isset($t08_penerimaan_rpt)) $t08_penerimaan_rpt = new crt08_penerimaan_rpt();
if (isset($Page)) $OldPage = $Page;
$Page = &$t08_penerimaan_rpt;
...
// Create page object
var t08_penerimaan_rpt = new ewr_Page("t08_penerimaan_rpt");

// Page properties
t08_penerimaan_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = t08_penerimaan_rpt.PageID;
</script>
<?php } ?>
...
// Form object
var CurrentForm = ft08_penerimaanrpt = new ewr_Form("ft08_penerimaanrpt");

// Validate method
ft08_penerimaanrpt.Validate = function() {
...
ft08_penerimaanrpt.Form_CustomValidate = 
...
ft08_penerimaanrpt.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
ft08_penerimaanrpt.ValidateRequired = false; // No JavaScript validation
<?php } ?>
...
<?php if ($Page->Export == "" && !$Page->DrillDown && !$grDashboardReport) { ?>
<!-- Search form (begin) -->
<form name="ft08_penerimaanrpt" id="ft08_penerimaanrpt" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="ft08_penerimaanrpt_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<script type="text/javascript">
ft08_penerimaanrpt.Init();
ft08_penerimaanrpt.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<!-- Report grid (begin) -->
<?php if ($Page->Export <> "pdf") { ?>
<div id="gmp_t08_penerimaan" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php } ?>
...
<?php if ($Page->SortUrl($Page->Tanggal) == "") { ?>
		<div class="ewTableHeaderBtn t08_penerimaan_Tanggal">
			<span class="ewTableHeaderCaption"><?php echo $Page->Tanggal->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t08_penerimaan_Tanggal" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Tanggal) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Tanggal->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="NoKwitansi"><div class="t08_penerimaan_NoKwitansi"><span class="ewTableHeaderCaption"><?php echo $Page->NoKwitansi->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="NoKwitansi">
<?php if ($Page->SortUrl($Page->NoKwitansi) == "") { ?>
		<div class="ewTableHeaderBtn t08_penerimaan_NoKwitansi">
			<span class="ewTableHeaderCaption"><?php echo $Page->NoKwitansi->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t08_penerimaan_NoKwitansi" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->NoKwitansi) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->NoKwitansi->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Keterangan"><div class="t08_penerimaan_Keterangan"><span class="ewTableHeaderCaption"><?php echo $Page->Keterangan->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Keterangan">
<?php if ($Page->SortUrl($Page->Keterangan) == "") { ?>
		<div class="ewTableHeaderBtn t08_penerimaan_Keterangan">
			<span class="ewTableHeaderCaption"><?php echo $Page->Keterangan->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t08_penerimaan_Keterangan" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Keterangan) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Keterangan->FldCaption() ?></span>
...
<?php include "t08_penerimaanrptpager.php" ?>
