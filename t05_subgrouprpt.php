...
<?php include_once "t05_subgrouprptinfo.php" ?>
<?php
...
$t05_subgroup_rpt = NULL; // Initialize page object first

class crt05_subgroup_rpt extends crt05_subgroup {
	...
	// Page object name
	var $PageObjName = 't05_subgroup_rpt';
	...
	function __construct() {
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 't05_subgroup', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption ft05_subgrouprpt";
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 't05_subgroup');
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewrExportLink ewEmail\" title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_t05_subgroup\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_t05_subgroup',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"ft05_subgrouprpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"ft05_subgrouprpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"ft05_subgrouprpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = FALSE;
	...
	function Page_Main() {
		...
		$this->maingroup_nama->SetVisibility();
	...
	function GetRow($opt) {
		...
		if (!$rs->EOF) {
			...
			$this->maingroup_nama->setDbValue($rs->fields('maingroup_nama'));
			$this->Nama->setDbValue($rs->fields('Nama'));
			$this->Val[1] = $this->maingroup_nama->CurrentValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			// maingroup_nama
			$this->maingroup_nama->ViewValue = $this->maingroup_nama->CurrentValue;
			$this->maingroup_nama->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// maingroup_nama
			$CurrentValue = $this->maingroup_nama->CurrentValue;
			$ViewValue = &$this->maingroup_nama->ViewValue;
	...
	function SetupFieldCount() {
		...
		if ($this->maingroup_nama->Visible) $this->DtlColumnCount += 1;
...
// Create page object
if (!isset($t05_subgroup_rpt)) $t05_subgroup_rpt = new crt05_subgroup_rpt();
if (isset($Page)) $OldPage = $Page;
$Page = &$t05_subgroup_rpt;
...
// Create page object
var t05_subgroup_rpt = new ewr_Page("t05_subgroup_rpt");

// Page properties
t05_subgroup_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = t05_subgroup_rpt.PageID;
</script>
<?php } ?>
...
<!-- Report grid (begin) -->
<?php if ($Page->Export <> "pdf") { ?>
<div id="gmp_t05_subgroup" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="maingroup_nama"><div class="t05_subgroup_maingroup_nama"><span class="ewTableHeaderCaption"><?php echo $Page->maingroup_nama->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="maingroup_nama">
<?php if ($Page->SortUrl($Page->maingroup_nama) == "") { ?>
		<div class="ewTableHeaderBtn t05_subgroup_maingroup_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->maingroup_nama->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t05_subgroup_maingroup_nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->maingroup_nama) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->maingroup_nama->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Nama"><div class="t05_subgroup_Nama"><span class="ewTableHeaderCaption"><?php echo $Page->Nama->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Nama">
<?php if ($Page->SortUrl($Page->Nama) == "") { ?>
		<div class="ewTableHeaderBtn t05_subgroup_Nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->Nama->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t05_subgroup_Nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Nama) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Nama->FldCaption() ?></span>
...
<span<?php echo $Page->maingroup_nama->ViewAttributes() ?>><?php echo $Page->maingroup_nama->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->Nama->Visible) { ?>
...
<?php include "t05_subgrouprptpager.php" ?>
