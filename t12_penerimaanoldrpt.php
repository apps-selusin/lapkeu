...
<?php include_once "t12_penerimaanoldrptinfo.php" ?>
<?php
...
$t12_penerimaanold_rpt = NULL; // Initialize page object first

class crt12_penerimaanold_rpt extends crt12_penerimaanold {
	...
	// Page object name
	var $PageObjName = 't12_penerimaanold_rpt';
	...
	function __construct() {
		...
		// Table object (t12_penerimaanold)
		if (!isset($GLOBALS["t12_penerimaanold"])) {
			$GLOBALS["t12_penerimaanold"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["t12_penerimaanold"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 't12_penerimaanold', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption ft12_penerimaanoldrpt";
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 't12_penerimaanold');
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewrExportLink ewEmail\" title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_t12_penerimaanold\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_t12_penerimaanold',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"ft12_penerimaanoldrpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"ft12_penerimaanoldrpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"ft12_penerimaanoldrpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['NoKwitansi'] = ewr_Conv($rs->fields('NoKwitansi'), 200);
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			$this->NoKwitansi->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// NoKwitansi
			$CurrentValue = $this->NoKwitansi->CurrentValue;
			$ViewValue = &$this->NoKwitansi->ViewValue;
			$ViewAttrs = &$this->NoKwitansi->ViewAttrs;
			$CellAttrs = &$this->NoKwitansi->CellAttrs;
			$HrefValue = &$this->NoKwitansi->HrefValue;
			$LinkAttrs = &$this->NoKwitansi->LinkAttrs;
			$this->Cell_Rendered($this->NoKwitansi, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_t12_penerimaanold_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_t12_penerimaanold_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_t12_penerimaanold_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_t12_penerimaanold_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_t12_penerimaanold_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_t12_penerimaanold_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_t12_penerimaanold_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_t12_penerimaanold_' . $parm] = $sv;
		$_SESSION['so_t12_penerimaanold_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_t12_penerimaanold_' . $parm] = $sv1;
		$_SESSION['so_t12_penerimaanold_' . $parm] = $so1;
		$_SESSION['sc_t12_penerimaanold_' . $parm] = $sc;
		$_SESSION['sv2_t12_penerimaanold_' . $parm] = $sv2;
		$_SESSION['so2_t12_penerimaanold_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_t12_penerimaanold_$parm"] = "";
		$_SESSION["rf_t12_penerimaanold_$parm"] = "";
		$_SESSION["rt_t12_penerimaanold_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->FieldByParm($parm);
		$fld->SelectionList = @$_SESSION["sel_t12_penerimaanold_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_t12_penerimaanold_$parm"];
		$fld->RangeTo = @$_SESSION["rt_t12_penerimaanold_$parm"];
	}
...
// Create page object
if (!isset($t12_penerimaanold_rpt)) $t12_penerimaanold_rpt = new crt12_penerimaanold_rpt();
if (isset($Page)) $OldPage = $Page;
$Page = &$t12_penerimaanold_rpt;
...
// Create page object
var t12_penerimaanold_rpt = new ewr_Page("t12_penerimaanold_rpt");

// Page properties
t12_penerimaanold_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = t12_penerimaanold_rpt.PageID;
</script>
<?php } ?>
...
// Form object
var CurrentForm = ft12_penerimaanoldrpt = new ewr_Form("ft12_penerimaanoldrpt");

// Validate method
ft12_penerimaanoldrpt.Validate = function() {
...
ft12_penerimaanoldrpt.Form_CustomValidate = 
...
ft12_penerimaanoldrpt.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
ft12_penerimaanoldrpt.ValidateRequired = false; // No JavaScript validation
<?php } ?>
...
<?php if ($Page->Export == "" && !$Page->DrillDown && !$grDashboardReport) { ?>
<!-- Search form (begin) -->
<form name="ft12_penerimaanoldrpt" id="ft12_penerimaanoldrpt" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="ft12_penerimaanoldrpt_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<script type="text/javascript">
ft12_penerimaanoldrpt.Init();
ft12_penerimaanoldrpt.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<!-- Report grid (begin) -->
<?php if ($Page->Export <> "pdf") { ?>
<div id="gmp_t12_penerimaanold" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php } ?>
...
<?php if ($Page->SortUrl($Page->Tanggal) == "") { ?>
		<div class="ewTableHeaderBtn t12_penerimaanold_Tanggal">
			<span class="ewTableHeaderCaption"><?php echo $Page->Tanggal->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t12_penerimaanold_Tanggal" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Tanggal) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Tanggal->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->NoKwitansi->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="NoKwitansi"><div class="t12_penerimaanold_NoKwitansi"><span class="ewTableHeaderCaption"><?php echo $Page->NoKwitansi->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="NoKwitansi">
<?php if ($Page->SortUrl($Page->NoKwitansi) == "") { ?>
		<div class="ewTableHeaderBtn t12_penerimaanold_NoKwitansi">
			<span class="ewTableHeaderCaption"><?php echo $Page->NoKwitansi->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t12_penerimaanold_NoKwitansi" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->NoKwitansi) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->NoKwitansi->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->NoKwitansi->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->NoKwitansi->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Keterangan"><div class="t12_penerimaanold_Keterangan"><span class="ewTableHeaderCaption"><?php echo $Page->Keterangan->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Keterangan">
<?php if ($Page->SortUrl($Page->Keterangan) == "") { ?>
		<div class="ewTableHeaderBtn t12_penerimaanold_Keterangan">
			<span class="ewTableHeaderCaption"><?php echo $Page->Keterangan->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t12_penerimaanold_Keterangan" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Keterangan) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Keterangan->FldCaption() ?></span>
...
<?php } ?>
<?php if ($Page->NoKwitansi->Visible) { ?>
		<td data-field="NoKwitansi"<?php echo $Page->NoKwitansi->CellAttributes() ?>>
<span<?php echo $Page->NoKwitansi->ViewAttributes() ?>><?php echo $Page->NoKwitansi->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->Keterangan->Visible) { ?>
...
<?php include "t12_penerimaanoldrptpager.php" ?>
